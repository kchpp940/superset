...
interface AnomalyRuleModalProps {
  chartId: number;
  dashboardId?: number;
  visible: boolean;
  onClose: () => void;
...
export const AnomalyRuleModal: FC<AnomalyRuleModalProps> = ({
  chartId,
  dashboardId,
  visible,
  onClose,
  ...
  const [ruleType, setRuleType] = useState<AnomalyRuleType>('threshold' as AnomalyRuleType);
  ...
  }, [visible, fetchExistingRules, editingRule, form]);
        ...
        return (
          <>
            <Form.Item
              ...
              rules={[{ required: true, message: t('Please enter change percent threshold') }]}
  ...
  return (
    <Card
      title={
        <Title level={4}>
          {editingRule ? t('Edit Anomaly Rule') : t('Configure Anomaly Detection')}
        </Title>
      }
      extra={
        <Space>
    ...
    >
      {existingRules.length > 0 && (
        <>
          <Title level={5}>{t('Existing Rules')}</Title>
          <List
            dataSource={existingRules}
            renderItem={rule => (
              <List.Item
                actions={[
                  <Button type="link" onClick={() => handleEdit(rule)}>
                    {t('Edit')}
                  </Button>,
                  <Popconfirm
              ...
              >
                <List.Item.Meta
                  ...
                  description={
                    <Text type="secondary">
                      {t('Metric:')} {rule.metric} | {t('Type:')} {rule.rule_type}
                      {rule.last_anomaly_at && (
                        <>
                          {' | '}
                          <Text type="warning">
                            {t('Last anomaly:')} {rule.last_anomaly_at}
                          </Text>
                        </>
                      )}
                    </Text>
        ...
        <Row gutter={16}>
          ...
          <Col span={12}>
            <Form.Item
              name="metric"
              label={t('Metric')}
              rules={[{ required: true, message: t('Please select metric') }]}
            >
              <Select placeholder={t('Select metric to monitor')} showSearch>
                {metrics.map(metric => (
...
export default AnomalyRuleModal;
